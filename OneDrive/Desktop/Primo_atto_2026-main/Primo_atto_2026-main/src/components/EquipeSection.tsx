...
import { trackEquipeClick } from '../utils/analytics';

export const EquipeSection = () => {
  const handleMembroClick = (nome: string, cargo: string) => {
    trackEquipeClick(nome, cargo);
  };
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {equipe.map((membro, index) => {
          ...
          return (
            <div 
              key={index} 
              className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100 hover:shadow-2xl transition-shadow cursor-pointer"
              onClick={() => handleMembroClick(membro.nome, membro.cargo)}
            >
